...
export default function WeeklyView({
  ...
  // Auto-scroll to today or selected date on load
  useEffect(() => {
    if (scrollRef.current) {
      const targetDate = selectedDate || todayStr;
      const targetIndex = weekDates.indexOf(targetDate);
      if (targetIndex >= 0) {
        const card = scrollRef.current.children[targetIndex + 1]; // +1 for padding div
        if (card) {
          card.scrollIntoView({
            behavior: 'smooth',
            block: 'nearest',
  ...
  return (
    <div className="w-full bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 overflow-hidden shadow-sm transition-colors duration-300">
      ...
      <div className="px-4 sm:px-6 py-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center justify-between gap-4">
          <div className="flex-1">
            <h3 className="text-base sm:text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
              <span className="text-indigo-500">📅</span> Weekly Overview
            </h3>
          ...
          <div className="flex items-center gap-4 bg-gray-50 dark:bg-gray-800/50 px-3 py-2 rounded-xl border border-gray-100 dark:border-gray-700">
            <div className="text-right">
              <div className="text-[9px] text-gray-500 dark:text-gray-400 uppercase tracking-tighter font-bold">Week Progress</div>
              <div className={`text-sm sm:text-base font-black ${weekPct === 100 ? 'text-green-500' : 'text-indigo-500 dark:text-indigo-400'}`}>
                {weekPct}%
              </div>
            </div>
        ...
        <div className="mt-4 h-1.5 w-full bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
          <motion.div
            className="h-full rounded-full"
            style={{ backgroundColor: weekPct === 100 ? '#10b981' : '#6366f1' }}
            initial={{ width: 0 }}
            animate={{ width: `${weekPct}%` }}
      ...
      <div className="md:hidden">
        <motion.div
          ...
          initial="hidden"
          animate="visible"
          className="flex overflow-x-auto snap-x snap-mandatory scroll-smooth hide-scrollbar px-4 py-4 gap-3"
        >
          {/* Padding start */}
          ...
          {weekDates.map((dateStr) => {
            ...
            return (
              <motion.button
                ...
                onClick={() => setSelectedDate(dateStr)}
                className={`
                  snap-center flex-shrink-0 w-[80vw] sm:w-[300px] p-4 rounded-2xl text-left 
                  transition-all duration-300 border-2 flex flex-col min-h-[220px]
                  ${isSelected 
                    ? 'bg-indigo-50 dark:bg-indigo-900/30 border-indigo-500 shadow-lg shadow-indigo-200 dark:shadow-none' 
                    : 'bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700 hover:border-indigo-300 dark:hover:border-indigo-700'}
                `}
              >
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <span className={`text-[10px] font-bold uppercase tracking-widest ${isToday ? 'text-indigo-500' : 'text-gray-400'}`}>
                      {dayNamesShort[d.getDay()]}
                    </span>
                    <div className="flex items-baseline gap-1">
                      <span className={`text-2xl font-black ${isSelected ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-800 dark:text-white'}`}>
                        {d.getDate()}
                      </span>
                      <span className="text-xs text-gray-400">{d.toLocaleDateString('en-US', { month: 'short' })}</span>
                    </div>
                  </div>
                  {isToday && (
                    <span className="px-2 py-0.5 rounded-md bg-indigo-500 text-white text-[9px] font-bold uppercase">Today</span>
                  )}
                </div>

                <div className="flex-1 space-y-2 mb-4">
                  {tasks.length === 0 ? (
                    <div className="h-full flex items-center justify-center opacity-40 italic text-xs text-gray-500 py-4">No tasks scheduled</div>
                  ) : (
                    tasks.slice(0, 3).map(task => (
                      <div key={task.id} className="flex items-center gap-2 text-[11px] text-gray-600 dark:text-gray-300 truncate">
                        <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: task.done ? '#10b981' : getSubjectColor(task.subject) }} />
                        <span className={task.done ? 'line-through opacity-50' : ''}>{task.topic}</span>
                      </div>
                    ))
                  )}
                  {tasks.length > 3 && <div className="text-[10px] font-bold text-indigo-400 pl-3.5">+{tasks.length - 3} more tasks</div>}
                </div>

                {tasks.length > 0 && (
                  <div className="pt-3 border-t border-gray-100 dark:border-gray-700">
                    <div className="flex justify-between items-center mb-1.5">
                      <span className="text-[10px] font-semibold text-gray-500 dark:text-gray-400">{stats.completed}/{stats.total} Tasks</span>
                      <span className={`text-xs font-bold ${stats.percentage === 100 ? 'text-green-500' : 'text-indigo-500'}`}>{stats.percentage}%</span>
                    </div>
        ...
        <div className="flex justify-center gap-1.5 py-4">
          {weekDates.map((dateStr, i) => (
            <div 
              key={dateStr} 
              className={`h-1.5 rounded-full transition-all duration-300 ${selectedDate === dateStr ? 'w-6 bg-indigo-500' : 'w-1.5 bg-gray-300 dark:bg-gray-600'}`} 
            />
          ))}
        </div>
      </div>
      ...
      <div className="hidden md:block">
        <motion.div
          ...
          initial="hidden"
          animate="visible"
          className="grid grid-cols-7 divide-x divide-gray-100 dark:divide-gray-800"
        >
          {weekDates.map((dateStr) => {
            const d = new Date(dateStr + 'T00:00:00');
            const tasks = getDayTasks(dateStr);
            ...
            return (
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                onClick={() => setSelectedDate(dateStr)}
                className={`p-4 text-left transition-all relative min-h-[180px] flex flex-col 
                  ${isSelected ? 'bg-indigo-50/50 dark:bg-indigo-900/20 ring-2 ring-inset ring-indigo-500' : 'hover:bg-gray-50 dark:hover:bg-gray-800/40'}
                `}
              >
                <div className="flex justify-between items-center mb-3">
                  ...
                  <span className={`text-sm font-black ${isToday ? 'bg-indigo-500 text-white w-6 h-6 flex items-center justify-center rounded-full' : 'text-gray-700 dark:text-gray-300'}`}>
                    {d.getDate()}
                  </span>
                </div>
                ...
                <div className="flex-1 space-y-2 mb-4">
                  {tasks.slice(0, 4).map(task => (
                    <div key={task.id} className="flex items-center gap-2 text-[10px] truncate text-gray-600 dark:text-gray-400">
                      <div className="w-1 h-1 rounded-full" style={{ backgroundColor: task.done ? '#10b981' : getSubjectColor(task.subject) }} />
                      <span className={task.done ? 'line-through opacity-50' : ''}>{task.topic}</span>
                    </div>
                  ))}
                  {tasks.length === 0 && <span className="text-[10px] italic opacity-30">Empty</span>}
                </div>
                ...
                {tasks.length > 0 && (
                  <div className="mt-auto pt-3 border-t border-gray-100 dark:border-gray-800">
                    <div className="flex justify-between text-[9px] font-bold mb-1">
                      ...
                      <span className={stats.percentage === 100 ? 'text-green-500' : 'text-indigo-500'}>{stats.percentage}%</span>
                    </div>
                    <div className="h-1 w-full bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                      <div className="h-full bg-indigo-500 transition-all duration-500" style={{ width: `${stats.percentage}%` }} />
                    </div>
                  </div>
                )}
              </motion.button>
