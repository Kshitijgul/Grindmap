import React from 'react';
import { motion } from 'framer-motion';

export default function NavLogo2({ size = 'default' }) {
  const sizes = {
    small: { icon: 28, fontSize: 15, gap: 6, underline: 12 },
    default: { icon: 36, fontSize: 20, gap: 8, underline: 18 },
    large: { icon: 48, fontSize: 28, gap: 10, underline: 24 },
  };

  const s = sizes[size] || sizes.default;

  return (
    <div className="flex items-center select-none" style={{ gap: s.gap }}>
      {/* ══════ SVG Icon ══════ */}
      <motion.svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 32 32"
        width={s.icon}
        height={s.icon}
        className="flex-shrink-0"
        initial={{ opacity: 0, scale: 0.4, rotate: -45 }}
        animate={{ opacity: 1, scale: 1, rotate: 0 }}
        transition={{ type: 'spring', stiffness: 180, damping: 14, delay: 0.05 }}
      >
        <defs>
          <linearGradient id="pc_barPurple" x1="0%" y1="100%" x2="0%" y2="0%">
            <stop offset="0%" stopColor="#6D28D9" />
            <stop offset="100%" stopColor="#8B5CF6" />
          </linearGradient>
          <linearGradient id="pc_barGreen" x1="0%" y1="100%" x2="0%" y2="0%">
            <stop offset="0%" stopColor="#059669" />
            <stop offset="100%" stopColor="#22C55E" />
          </linearGradient>
          <linearGradient id="pc_barCoral" x1="0%" y1="100%" x2="0%" y2="0%">
            <stop offset="0%" stopColor="#EA580C" />
            <stop offset="100%" stopColor="#FB923C" />
          </linearGradient>
        </defs>

        {/* Background */}
        <rect
          width="32"
          height="32"
          rx="8"
          className="fill-white dark:fill-gray-900 stroke-gray-200 dark:stroke-gray-700"
          strokeWidth="0.6"
        />

        {/* Base Ring */}
        <motion.circle
          cx="16" cy="16" r="12"
          fill="none"
          className="stroke-indigo-500/40 dark:stroke-indigo-400/30"
          strokeWidth="3.5"
          initial={{ pathLength: 0, opacity: 0 }}
          animate={{ pathLength: 1, opacity: 1 }}
          transition={{ duration: 1.2, ease: 'easeInOut', delay: 0.2 }}
        />

        {/* Gold Arc */}
        <motion.path
          d="M4 16 A12 12 0 0 1 28 16"
          fill="none"
          stroke="#FFB900"
          strokeWidth="3.5"
          strokeLinecap="round"
          initial={{ pathLength: 0 }}
          animate={{ pathLength: 1 }}
          transition={{ duration: 0.9, ease: 'easeOut', delay: 0.6 }}
        />

        {/* Arrow Line */}
        <motion.path
          d="M28 16 L28 5"
          fill="none"
          className="stroke-indigo-500 dark:stroke-indigo-400"
          strokeWidth="3.5"
          strokeLinecap="round"
          initial={{ pathLength: 0 }}
          animate={{ pathLength: 1 }}
          transition={{ duration: 0.5, delay: 1.0 }}
        />

        {/* Arrow Head */}
        <motion.polygon
          points="28,2 25,6 31,6"
          className="fill-indigo-500 dark:fill-indigo-400"
          initial={{ opacity: 0, y: 5 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ type: 'spring', stiffness: 300, damping: 12, delay: 1.3 }}
        />

        {/* Growth Bars */}
        <motion.rect
          x="7" y="19" width="5" height="7" rx="2"
          fill="url(#pc_barPurple)"
          initial={{ scaleY: 0 }}
          animate={{ scaleY: 1 }}
          style={{ transformOrigin: '9.5px 26px' }}
          transition={{ type: 'spring', stiffness: 250, damping: 15, delay: 0.3 }}
        />
        <motion.rect
          x="13" y="14" width="5" height="12" rx="2"
          fill="url(#pc_barCoral)"
          initial={{ scaleY: 0 }}
          animate={{ scaleY: 1 }}
          style={{ transformOrigin: '15.5px 26px' }}
          transition={{ type: 'spring', stiffness: 250, damping: 15, delay: 0.5 }}
        />
        <motion.rect
          x="19" y="9" width="5" height="17" rx="2"
          fill="url(#pc_barGreen)"
          initial={{ scaleY: 0 }}
          animate={{ scaleY: 1 }}
          style={{ transformOrigin: '21.5px 26px' }}
          transition={{ type: 'spring', stiffness: 250, damping: 15, delay: 0.7 }}
        />
      </motion.svg>

      {/* ══════ Text ══════ */}
      <div className="flex flex-col">
        <motion.div
          className="flex items-baseline leading-none"
          initial={{ opacity: 0, x: -12 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 0.5, delay: 0.4 }}
        >
          <span
            className="font-black tracking-tight text-gray-900 dark:text-white"
            style={{ fontSize: s.fontSize, lineHeight: 1.15 }}
          >
            GRIND
          </span>
          <span
            className="font-black tracking-tight text-gray-400 dark:text-gray-500"
            style={{ fontSize: s.fontSize, lineHeight: 1.15 }}
          >
            MAP
          </span>
        </motion.div>

        {/* Animated Underline */}
        <div className="flex items-center gap-[3px] mt-[3px]">
          <motion.div
            className="h-[2.5px] rounded-full bg-indigo-500 dark:bg-indigo-400"
            initial={{ width: 0 }}
            animate={{ width: s.underline * 1.2 }}
            transition={{ duration: 0.4, delay: 0.7, ease: 'easeOut' }}
          />
          <motion.div
            className="h-[2.5px] rounded-full bg-purple-500 dark:bg-purple-400"
            initial={{ width: 0 }}
            animate={{ width: s.underline * 1.0 }}
            transition={{ duration: 0.35, delay: 0.9, ease: 'easeOut' }}
          />
          <motion.div
            className="h-[2.5px] rounded-full bg-pink-500 dark:bg-pink-400"
            initial={{ width: 0 }}
            animate={{ width: s.underline * 0.8 }}
            transition={{ duration: 0.3, delay: 1.1, ease: 'easeOut' }}
          />
        </div>
      </div>
    </div>
  );
}