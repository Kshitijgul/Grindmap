    ...
    <div className="bg-gray-900 rounded-2xl border border-gray-800 p-5">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => setCollapsed(!collapsed)}
            className="text-[10px] px-2.5 py-1 rounded-lg bg-gray-800 border border-gray-700 text-gray-500 hover:text-white hover:border-gray-600 transition font-medium"
          >
            {collapsed ? '▼ Show' : '▲ Hide'}
          </button>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {subjectsWithGoals.map(subject => {
            ...
            return (
              <div
                key={subject.id}
                className="flex items-center gap-2 px-3 py-1.5 bg-gray-800/60 border border-gray-700/50 rounded-xl"
              >
                <div className="w-2 h-2 rounded-full" style={{ backgroundColor: subject.color }} />
                <span className="text-xs text-gray-300 font-medium">{subject.name}</span>
                <span className={`text-[10px] font-bold ${
                  isCompleted ? 'text-green-400' : isExpired ? 'text-red-400' : 'text-indigo-400'
                }`}>
                  {stats.percentage}%
                </span>
              </div>
